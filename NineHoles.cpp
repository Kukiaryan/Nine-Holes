...
void NineHoles::showAbout() {
    QMessageBox::information(this, tr("About"), tr("Nine Holes\n\nAryan Kukreja -kukrejaaryan123@gmail.com"));
}

void NineHoles::showGameOver(Player player) {
    switch (player) {
        case NineHoles::RedPlayer:
            QMessageBox::information(this, tr("Game Over"), tr("Congratulations, Red Player wins the Game."));
            break;
        case NineHoles::BluePlayer:
            QMessageBox::information(this, tr("Game Over"), tr("Congratulations, Blue Player wins the Game."));
            break;
        default:
            Q_UNREACHABLE();
    }
...
void NineHoles::updateStatusBar() {
    QString player(m_player == NineHoles::RedPlayer ? "Red" : "Blue");
    QString phase(m_phase == NineHoles::DropPhase ? "Place" : "Move");

    ui->statusbar->showMessage(tr("Phase %1: Player's Turn: %2").arg(phase).arg(player));
}
